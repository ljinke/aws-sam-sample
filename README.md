...
This is a sample template for temp-conv - Below is a brief explanation of what we have generated for you:

```bash
.
├── README.MD                   <-- This instructions file
├── event.json                  <-- API Gateway Proxy Integration event payload
├── dependencies.json           <-- Modules that would be built as a Layer
│   └── nodejs                  <-- Structure required by SAM
...
```bash
sam local invoke TempConversionFunction --event event.json
```
...
```bash
sam logs -n HelloWorldFunction --stack-name temp-conv --tail
```
...
```bash
aws cloudformation delete-stack --stack-name temp-conv
```
